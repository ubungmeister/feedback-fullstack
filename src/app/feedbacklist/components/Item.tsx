...
export const Item = ({ feedback, refreshData }: ItemProps) => {
    ...
    const handleUpdate = async () => {
        const response = await fetch("/api/feedback", {
            ...
            body: JSON.stringify({ feedback: comment, id: feedback.id }),
        });
    ...
    return (
        <div className="border border-gray-400 p-3 rounded-lg">
            ...
            {isEditing ? (
                ...
                <p className="mt-2 break-words w-full overflow-hidden">{feedback.comment}</p>
            )}

            {feedback.rating && <p className="text-sm text-gray-600">Rating: {feedback.rating}</p>}
